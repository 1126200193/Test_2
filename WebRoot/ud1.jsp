...
number=new String(number.getBytes("ISO-8859-1"));
 ...
 try {
	   //连接数据库
	     ...
	     ResultSet rs=sql.executeQuery("select * from admin2 where number='"+number+"' and idcard='"+idcard+"'");
	     if(rs.next()){	    	   
	    	String condition="update admin2 set password='"+password+"' where number="+number;
		    sql.executeUpdate(condition);
		    out.print("<script LANGUAGE='javascript'>alert('修改成功！！');window.location.href='Entry1.jsp';</script>"); 
	     }
	     else{
	    	 ResultSet rs1=sql.executeQuery("select * from admin2 where number='"+number+"'");
	    	 if(rs1.next()){
	    		 out.print("<script LANGUAGE='javascript'>alert('对不起！您的身份证信息有误！修改失败！');window.location.href='Entry1.jsp';</script>"); 	 
	    	 }
	    	 else{
	    		 out.print("<script LANGUAGE='javascript'>alert('对不起，该用户名账号不存在！！');window.location.href='Entry1.jsp';</script>"); 
	    	 }
